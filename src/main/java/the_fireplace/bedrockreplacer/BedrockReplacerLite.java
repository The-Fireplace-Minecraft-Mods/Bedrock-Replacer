...
import the_fireplace.bedrockreplacer.events.LiteFMLEvents;
import the_fireplace.bedrockreplacer.events.LiteForgeEvents;

/**
 * @author The_Fireplace
 */
@Mod(modid=BedrockReplacer.MODID+"lite", name=BedrockReplacer.MODNAME+" Lite", version=BedrockReplacer.VERSION, acceptedMinecraftVersions = "1.8", canBeDeactivated=true)
public class BedrockReplacerLite {
	@Instance(BedrockReplacer.MODID+"lite")
	public static BedrockReplacerLite instance;
	...
	public void init(FMLInitializationEvent event){
		if(BRConfigValues.ENABLELITEMODE){
			FMLCommonHandler.instance().bus().register(new LiteFMLEvents());
			MinecraftForge.EVENT_BUS.register(new LiteForgeEvents());
		}
	}
	@EventHandler
